    ...
    private static synchronized GenericObjectPool<TrackerServer> getObjectPool() {
        if (trackerServerPool == null) {
            try {
                ...
                //String configFile = TrackerServerPool.class.getClassLoader().getResource(FASTDFS_CONFIG_PATH).getPath();
                //String configFile = FastDFSClient.class.getResource("/").getPath() + FASTDFS_CONFIG_PATH;
                // 加载配置文件
