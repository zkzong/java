...
import org.example.job.MyDataFlowJob;
import org.example.job.MySimpleJob;
...
public class ElasticJobConfig {
    ...
    public JobScheduler scriptJobScheduler(@Value("${gupaoJob.script.cron}") final String cron,
        ...
        File file = null;
        try {
            file = ResourceUtils.getFile("classpath:1.bat");
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
        String absolutePath = file.getAbsolutePath();
        return new SpringJobScheduler(null, regCenter,
                getScriptJobConfiguration("script_job", cron, shardingTotalCount, shardingItemParameters, absolutePath));
    }
